...
export const replaceString = (x: string | undefined) => {
  if (x) {
    const initialString = x;
    console.log(initialString, 'string')
    const showString = initialString.replace(initialString,initialString.slice(2));
    return showString;
  } else {
    return;
  }
...
export const validationSchema = Yup.object({
  ...
  clientAddress: Yup.object({
    name: Yup.string().max(20).required("Name is required, please add a name"),
    email: Yup.string()
